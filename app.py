...
st.set_page_config(page_title="matbot is online", layout="centered")

@st.cache_resource
def load_model():
    from huggingface_hub import login
    login(st.secrets["hf_token"])
    tokenizer = GPT2Tokenizer.from_pretrained("wifeemailer227/gpt2-mat", use_auth_token=True)
    model = GPT2LMHeadModel.from_pretrained("wifeemailer227/gpt2-mat", use_auth_token=True)
...
with st.container():
    for entry in st.session_state.history:
        if entry["sender"] == "user":
            ...
            st.markdown(f"<div class='chat-bubble matbot left'>{entry['text']}</div>", unsafe_allow_html=True)
    if st.session_state.typing and not st.session_state.pending_chunks:
        st.markdown("<div class='chat-bubble matbot left typing-bubble'>matbot is typing...</div>", unsafe_allow_html=True)
...
if st.session_state.typing:
    if not st.session_state.pending_chunks:
        if not st.session_state.current_reply:
            ...
            final_phrase = random.choice(["legit", "legittt", "person", "what", "wtf", "yaknow", "bruh", "omg", "fuck", "gay", "fuck my life"])
...
if save_clicked:
    ...
    with open(filename, "w") as f:
        for entry in st.session_state.history:
            prefix = "you:" if entry["sender"] == "user" else "matbot:"
